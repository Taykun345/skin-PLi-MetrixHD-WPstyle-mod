...
class MetrixExtendedServiceInfo(Converter, object):
	...
	def getText(self):
		...
		if self.type == self.SERVICENAME:
			...
			if config.plugins.MetrixExtendedServiceInfo.showServiceNumber.value == True and number != "":
				text = "%s. %s" % (number, name)
			else:
				text = name
			if config.plugins.MetrixExtendedServiceInfo.showOrbitalPosition.value == True and orbital != "":
				if config.plugins.MetrixExtendedServiceInfo.orbitalPositionType.value == "name":
